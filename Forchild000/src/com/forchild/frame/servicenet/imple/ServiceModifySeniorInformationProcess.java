...
public class ServiceModifySeniorInformationProcess implements ServiceNetworkResultHandler {
	protected Preferences preference;
	protected Context context;

	public ServiceModifySeniorInformationProcess() {

	}

	public ServiceModifySeniorInformationProcess(Context context, Preferences preference) {
	...
	@Override
	public int getType() {
		return BaseProtocolFrame.MODIFY_SENIOR_INFORMATION;
	}

	@Override
	public int process(BaseProtocolFrame source) {
		if (source != null && source instanceof RequestModifySeniorInformation) {
			switch (source.getReq()) {
			case BaseProtocolFrame.RESPONSE_TYPE_OKAY:

				Toast.makeText(context, context.getText(R.string.response_error_modify_senior_infomation_okay), Toast.LENGTH_SHORT).show();
