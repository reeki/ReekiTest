...
public class ServiceSeniorLocationProcess implements ServiceNetworkResultHandler {

	protected Context context;
	protected Preferences preference;

	public ServiceSeniorLocationProcess() {

	}

	public ServiceSeniorLocationProcess(Context context, Preferences preference) {
	...
	public int process(BaseProtocolFrame source) {
		if (source != null && source instanceof RequestSeniorLocation) {
			switch (source.getReq()) {
			case BaseProtocolFrame.RESPONSE_TYPE_OKAY:
				Toast.makeText(context, context.getText(R.string.response_error_senior_location_okay), Toast.LENGTH_SHORT).show();
				break;
			...
			case BaseProtocolFrame.RESPONSE_TYPE_NULL_DATA:
				Toast.makeText(context, context.getText(R.string.response_error_senior_location_nodata), Toast.LENGTH_SHORT).show();
				break;
			case BaseProtocolFrame.RESPONSE_TYPE_OTHER_LOGIN:
				...
				break;
			default:
				Toast.makeText(context, context.getText(R.string.response_error_senior_location_fault), Toast.LENGTH_SHORT).show();
				break;
			}
		}
		return 0;
